...
class DragRaceGUI:
    def __init__(self, simulation_0, simulation_1=None):
        ...
        Params
        -------
        - `simulation_0` is the player's Run object
        - `simulation_1` is the bots's Run object
        - 
        """
        ...
        # background image
        self.bg = pg.image.load('images/road.png')
        self.bg_X = 0 #initial X position (horizontal)

        self.font = pg.font.Font(None, 32)

        self.car_sound = None
        self.init_car_sound_obj()
        ...
        if self.simulations[0].car.forced_induction == 1:
        ...
        #car image parameters
        self.car_name = self.simulations[0].car.get_name()
        self.car_image = pg.image.load('images/'+self.car_name+'/'+self.car_name+'.png')
        self.wheel_image = pg.image.load('images/'+self.car_name+'/'+self.car_name+'_wheels_static.png')

    def init_car_sound_obj(self):
    ...
    def draw_bg(self):
        ...
        self.bg_X -= self.simulations[0].current_speed
        if self.bg_X <= -width:
            self.bg_X = 0
    ...
    def draw_car(self):
        self.car_image = pg.transform.scale(self.car_image, (700, 280))
        self.screen.blit(self.car_image, dest=(0, 400))
        if self.launched:
            rng = random.randint(0, 1)
            self.wheel_image = pg.image.load('images/'+self.car_name+'/'+self.car_name+'_wheels_rot_'+str(rng)+'.png')
            self.wheel_image = pg.transform.scale(self.wheel_image, (700, 280))
            self.screen.blit(self.wheel_image, dest=(0, 400))
        else:
            self.wheel_image = pg.image.load('images/'+self.car_name+'/'+self.car_name+'_wheels_static.png')
            self.wheel_image = pg.transform.scale(self.wheel_image, (700, 280))
            self.screen.blit(self.wheel_image, dest=(0, 400))
    ...
    def draw_car_info(self):
        for which in [0,1]:
            ...
            name_text = self.font.render(f"{car_name}", True, (255, 255, 255))
            self.screen.blit(name_text, (60+which*640, 2))

            rpm_text = self.font.render(f"RPM: {int(self.simulations[which].current_rpm)}", True, (255, 255, 255))
            self.screen.blit(rpm_text, (60+which*640, 70))

            speed_text = self.font.render(f"Speed: {int(self.simulations[which].current_speed * 3.6)} km/h", True, (255, 255, 255))
            self.screen.blit(speed_text, (20+which*640, 120))

            time_text = self.font.render(f"Time: {int(self.simulations[which].current_seconds)}", True, (255, 255, 255))
            self.screen.blit(time_text, (20+which*640, 150))

            gear_text = self.font.render(f"{self.simulations[which].gear_index + 1}", True, (255, 255, 255))
            self.screen.blit(gear_text, (10+which*640, 70))
    ...
    def run(self):
        ...
        while running:
            ...
            if self.launched:
            ...
            elif not self.launched: # rpm  control
                if keys[pg.K_UP]:
                ...
                print('car has launched')

            #self.update_display() -> if i move it here under the above block no window shows up
            # check for end
...
class CarSound:
    def __init__(self, engine_sound_file, simulation):
        ...
        Attributes
        ----------
        - `self.spool`: the spool sound
        - `self.blow_off`: the blowoff valve sound
        - `self.final_sound_array`: the array of arrays of the engine sounds for each rpm interval
        - `self.sound_array_rpms`: the base engine sound in array form
        """
    ...
    def load_turbo_sounds(self):
        """assigns values to self.spool and self.blow_off"""
    ...
    def make_sound_array(self):
        ...
        for rpm in np.linspace(min_rpm, max_rpm, 120):
            # data
            print("rpm ", rpm)
    ...
    def turbo_sound(self):
        if self.simulations[0].car.forced_induction != 0 and self.channels[6].get_busy()==False and self.simulations[0].stututu == True:
                self.channels[6].play(self.blow_off)
                if not (self.spool is None) :self.channels[6].queue(self.spool)
                self.simulations[0].stututu_done()
...
car_name_1 = "Nismo_400R"
car_name= "Citroen_Saxo_VTS_Custom"
car_name_0 = "2013_Peugeot_208_GTI"
